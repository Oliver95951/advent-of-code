...
def question_count(answer_list: list):
    new_list = list(set(answer_list))
    count = 0
...
sum = 0
for x in clean_data:
    sum += (question_count(x))

print(sum)

raw_data_split = raw_data.split('\n\n')
clean_data_number_2 = []
for x in raw_data_split:
    clean_data_number_2.append(x.split('\n'))

def answer_count_grouped(list_lists: list):
    sum = 0

    for group in list_lists:
        if len(group) == 1:
            sum += len(group[0])
        else:
            intersection_of_answers = set(group[0]).intersection(*group[1:])
            sum += len(intersection_of_answers)

    return sum

print(answer_count_grouped(clean_data_number_2))
