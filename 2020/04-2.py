...
raw_input = """eyr:2024 pid:662406624 hcl:#cfa07d byr:1947 iyr:2015 ecl:amb hgt:150cm
...
split_input = [string.replace('\n',' ') for string in split_input]
split_input = [item.split() for item in split_input]

def convert_to_dict(pp_list):
    dictionary = {}
...
passport_dict = [convert_to_dict(item) for item in split_input]
...
valid_passport = [passport for passport in passport_dict if valid_passport(passport)]
print(len(valid_passport))

def has_valid_values(passport):
    valid_birth_year = 1920 <= int(passport['byr']) <= 2002
    valid_issue_year = 2010 <= int(passport['iyr']) <= 2020
    valid_expiration_year = 2020 <= int(passport['eyr']) <= 2030
    ...
    valid_hair_colour = False
    if len(passport['hcl']) == 7:
        digits = passport['hcl'][1:]
        valid_hair_colour = is_valid_hex_string(digits)

    valid_eye_colours_list = ['amb','blu','brn','gry','grn','hzl','oth']
    valid_eye_colour = passport['ecl'] in valid_eye_colours_list

    def has_valid_pp_ID(value):
        is_valid = False
    ...
    valid_pp_ID = has_valid_pp_ID(passport['pid'])
    ...
    return (
        ...
        valid_height and
        valid_hair_colour and
        valid_eye_colour and
        valid_pp_ID
    )

valid_passports = [passport for passport in valid_passport if has_valid_values(passport)]
