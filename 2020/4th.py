...
def valid_pp(x: str):
    ...
    if find_byr(x) and find_iyr(x) and find_eyr(x) and find_hgt(x) and find_hcl(x) and find_ecl(x) and find_pid(x):
        valid = True
...
valid_pp_count = 0
for index, x in enumerate(clean_data):
    if valid_pp(x):
        print(f'{index}:    This passport is Valid')
        valid_pp_count += 1
    else:
        print(f'{index}:    This passport is not Valid')

print(f'There are {valid_pp_count} valid passports out of {len(clean_data)}')